...
if (!window.DM) {
    DM = {
        ...
        _logging: true,
        _domain: {api: "https://api.dailymotion.com", www: "//www.dailymotion.com", cdn: "//api.dmcdn.net"},
        _oauth: {
        ...
        create: function (d, g) {
            var f = window.DM, a = d ? d.split(".") : [], j = a.length;
            for (var e = 0; e < j; e++) {
                var b = a[e];
                var h = f[b];
                if (!h) {
...
DM.provide("ApiServer", {
    type: null, METHODS: ["get", "post", "delete"], _callbacks: {}, _calls: [], call: function () {
        ...
        var d = DM.QS.encode(b), c = d.length ? (a.indexOf("?") > -1 ? "&" : "?") + d : "";
        ...
        var c = {path: d, method: f, params: e, cb: a};
        if (!b) {
        ...
        e.onreadystatechange = function () {
            if (e.readyState == 4) {
                var h = {error: {code: 500, message: "Invalid server response", type: "transport_error"}}, i;
                if (e.status) {
                    try {
        ...
        var c = [], h = DM._domain.api;
        for (var b = 0, a = d.length; b < a; b++) {
            var e = d[b];
            c.push({call: e.method.toUpperCase() + " /" + e.path, args: e.params, id: b})
        }
        if (DM._session && DM.Auth.isSessionExpired()) {
        ...
        g.onreadystatechange = function () {
            if (g.readyState == 4) {
                var j = {error: {code: 500, message: "Invalid server response", type: "transport_error"}}, q;
                if (g.status) {
                    try {
    ...
    getLoginStatus: function (a) {
        if (a) {
            DM.Auth.refreshToken(DM._session, function (b) {
                a({status: DM._userStatus, session: b})
            })
        }
    }, getSession: function () {
        ...
        var f = typeof window.screenX != "undefined" ? window.screenX : window.screenLeft,
            ...
            b = 500, l = 520, e = parseInt(f + ((m - b) / 2), 10), i = parseInt(d + ((j - l) / 2.5), 10),
...
DM.provide("Auth", {
    _active: {}, _receivedSession: null, loadSiteSession: function () {
        var c = true;
        var f = {};
        var b = {session: null, loading_method: null};
        ...
        if (DM._apiKey && DM._apiSecret && c && c.refresh_token) {
            ...
            e.onreadystatechange = function () {
                if (e.readyState == 4) {
                    var g = {error: {code: 500, message: "Invalid server response", type: "transport_error"}}, h;
                    if (e.status) {
                        try {
        ...
        var b = window.location.hash.replace("%23", "#"), a = b.substr(b.lastIndexOf("#") + 1);
        ...
        var g = !DM._session && e, i = DM._session && !e, f = false,
            c = g || i || (DM._session && e && DM._session.access_token != e.access_token), d = b != DM._userStatus;
        if (e && "expires_in" in e) {
            e.expires = Math.round(new Date().getTime() / 1000) + parseInt(e.expires_in, 10);
            var h = parseInt(e.expires_in, 10);
            delete e.expires_in
        }
        var a = {session: e, status: b};
        DM._session = e;
        DM._userStatus = b;
        ...
        for (var f in DM.Auth._active) {
            ...
            if ("session" in DM.Auth._active[f]) {
                ...
                if ("cb" in a) {
                    a.cb({status: DM._userStatus, session: DM._session})
                }
            }
        }
        var b = false;
...
DM.provide("Player", {
    ...
    create: function (d, b) {
        d = DM.$(d);
        if (!d || d.nodeType != 1) {
            throw new Error("Invalid first argument sent to DM.player(), requires a HTML element or element id: " + d)
        }
        if (!b || typeof b != "object") {
            throw new Error("Missing `options' parameter for DM.player()")
        }
        b = DM.copy(b, {width: 480, height: 270, title: "video player", params: {}, events: {}});
    ...
    init: function (b, c) {
        ...
        this.id = c.id = this.id ? this.id : DM.guid();
        this.src = DM.Player._PROTOCOL + DM._domain.www + "/embed" + (b ? "/video/" + b : "") + "?" + DM.QS.encode(c);
        ...
        this.autoplay = DM.parseBool(c.autoplay)
    },
    ...
    _send: function (c, a) {
        if (!this.apiReady) {
            try {
                if (console && typeof console.warn === "function") {
                    console.warn('Player not ready. Ignoring command : "' + c + '"')
                }
            } catch (b) {
            }
            return
        ...
        if (DM.Player.API_MODE == "postMessage") {
            this.contentWindow.postMessage(JSON.stringify({command: c, parameters: a || []}), DM.Player._IFRAME_ORIGIN)
        }
    },
    _dispatch: document.createEvent ? function (a) {
        ...
        if ("on" + a in this) {
            ...
            if (a in this.EVENT_HANDLERS) {
                var b = {type: a, target: this};
                DM.Array.forEach(this.EVENT_HANDLERS[a], function (c) {
                    c(b)
                })
            }
        }
    ...
    _recvEvent: function (a) {
        switch (a.event) {
            ...
            case"pause":
                ...
                break;
            case"error":
                this.error = {code: a.code, title: a.title, message: a.message};
                break;
            case"rebuffer":
            ...
            case"subtitlechange":
                ...
                break;
            case"videochange":
                this.video = {videoId: a.videoId, title: a.title};
                break
        }
        this._dispatch(a.event)
    },
